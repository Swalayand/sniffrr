...
void* printEvent(void *arg){	
	while(1){
		...
		for (int i = 0; i < 54; i++){	
			for (int j = 0; j < 20; j++)
				if ((ar[i] == full_digits[j] && ar[13] == 2) || (ar[i] == full_digits[j] && ar[13] == 3)) printf("%2d ", j%10 ); fflush( stdout );
			if (i % 18 == 0) printf("\t");
		}
...
/*
Membuka kemungkinan kalau solusinya bisa jadi sebaris, tapi siap untuk elaborasi berapa baris pun. 

Menyelipkan kalimat baru di mana? 
