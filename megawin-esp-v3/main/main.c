...
/* 
Code ini adalah server serial.
Kelemahan:
1. Angka log dari megawin nggak pernah berubah ketika barang berubah.

*/
#define EXAMPLE_ESP_WIFI_SSID      "Catetin Timbangan"//CONFIG_ESP_WIFI_SSID
#define EXAMPLE_ESP_WIFI_PASS      "Passwordnyatimbangan31"//CONFIG_ESP_WIFI_PASSWORD
	...
	while(1){
		...
		if (shbf_done == 1 && clk_pin.state == RISING){
			...
			if ( (car+1) % 54 == 0 && car != 0 ) {
                printEvent(NULL);
                shbf_done = 0;
                return; // kalo nggak ada return di sini error guru meditation
            }
		}
	}
}
...
static void do_retransmit(const int sock)
{
    ...
    char rx_buffer[1024];
    char tx_buffer[1] = "\n";//= "HTTP 200 OK\r\n\r\n";
    strcat(res, tx_buffer);
    // len = recv(sock, rx_buffer, sizeof(rx_buffer) - 1, 0);
    // send(sock, tx_buffer, strlen(tx_buffer), 0);
    // while (1){
    
    send_d = send(sock, res, strlen(res), 0);   
    printf("send\n"); 
    // if (send_d < 1) return;    
