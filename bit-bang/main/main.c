...
void read_bitbybit(void *pvParameter) {
	...
	char *state[] = { "LOW", "RISING", "FALING", "HIGH", "BLANK" };
	int i = 0;
	for(i=0; i < AMAX; i++) { 
	...
	for(i = 0; i < AMAX; i++) {
	  ...
	  stbs =  stb | (stb0 << 1); // 
		...
		/*
		bit-shift diatas diimplementasi dalam bentuk yang tidak efisien dalam 4 baris ini.

		if (stb == 1 && stb0 == 0) stbs = 01;
		if (stb == 0 && stb0 == 1) stbs = 10;
		if (stb == 1 && stb0 == 1) stbs = 11;
		if (stb == 0 && stb0 == 0) stbs = 00;
		*/
