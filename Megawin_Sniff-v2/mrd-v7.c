...
void* printEvent(void *arg){	
	...
	*/
	while(1){
		...
		//if ( /*ar[3] != 0 ||*/ ar[6] != 0 || ar[8] != 0 || ar[10] != 0 ) continue; // element 6,8,10 wajib = 0
		/*
		for (int i = 0; i < 18; i++){	
			printf("%3d ", ar[i] );
			/*
...
void eventCount(){
	...
	while(1){
		...
		if (stb_pin_prev.ctr > 40000 && stb_pin.state == FALLING) shbf_done = 1;	// dengan baris 117 ini seperti logika AND yang artinya memperhatikan proses awal dan akhir pengambilan data		
